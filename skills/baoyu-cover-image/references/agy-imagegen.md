...
- **`--ref` (up to 3 images)** holds character/subject appearance across generations reliably — pass a prior generation (or a character sheet) as `--ref` and the prompt does not need to redescribe hair/outfit/face details.
...
- `generate_image` returns **one image per call** (`n=1` only). Multi-image jobs must dispatch one wrapper call per image.
