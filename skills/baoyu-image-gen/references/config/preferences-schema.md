...
default_provider: null      # google|vertex|openai|azure|openrouter|dashscope|siliconflow|zai|minimax|replicate|jimeng|seedream|codex-cli|agy-cli|agnes|null (null = auto-detect; codex-cli and agy-cli are never auto-detected — pin them here or via --provider)
...
| `default_model.agy-cli` | string\|null | null | agy `--model` to run under (default `gemini-3.7-flash-medium`) |
