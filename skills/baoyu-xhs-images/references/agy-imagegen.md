...
- **`--ref` (up to 3 images)** holds character/subject appearance across generations reliably — pass image 1 (or a character sheet) as `--ref` for images 2+ and the prompt does not need to redescribe mascot/color/rendering details.
...
- `generate_image` returns **one image per call** (`n=1` only). Multi-image jobs must dispatch one wrapper call per card, per the `## Batch Generation Policy` in SKILL.md.
- The wrapper does NOT accept a `--sessionId` flag. Chain/scene consistency must come from `--ref` reference images — image 1 as `--ref` for every subsequent card, per Step 3 of the workflow.
