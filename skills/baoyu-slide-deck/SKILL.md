...
1. Resolve the image backend via the Image Generation Tools rule at the top — ask once if multiple are installed.
   ...
   - **`agy-imagegen` invocation**: when the rule resolves to `agy-imagegen`, see [references/agy-imagegen.md](references/agy-imagegen.md) for the invocation contract (preferred `baoyu-image-gen --provider agy-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics — n=1 per call so slide batches must dispatch one wrapper call per slide).
...
- **Common one-line edits**:
  ...
  - `preferred_image_backend: agy-imagegen` — pin to Antigravity's built-in.
