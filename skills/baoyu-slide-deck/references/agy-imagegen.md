...
- **`--ref` (up to 3 images)** holds visual/brand consistency across slides reliably — pass a prior slide or a style/brand reference as `--ref` and the prompt does not need to redescribe every visual detail.
...
- `generate_image` returns **one image per call** (`n=1` only). Multi-image jobs must dispatch one wrapper call per slide.
